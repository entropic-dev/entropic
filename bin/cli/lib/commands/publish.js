...
async function publish (opts) {
  ...
  if (packagejson.private) {
    opts.log.error('This package.json is marked private. Cowardly refusing to publish.')
    return 1
  }
  ...
  const pkgReq = await fetch(`${opts.registry}/packages/package/${packagejson.name}`)

  const mustCreate = pkgReq.status === 404

  if (mustCreate) {
    const request = await fetch(`${opts.registry}/packages/package/${packagejson.name}`, {
  ...
  form.append('devDependencies', JSON.stringify(packagejson.devDependencies || {}))
  form.append('optionalDependencies', JSON.stringify(packagejson.optionalDependencies || {}))
  form.append('peerDependencies', JSON.stringify(packagejson.peerDependencies || {}))
  form.append('bundledDependencies', JSON.stringify(packagejson.bundledDependencies || {}))
  ...
  const request = await fetch(`${opts.registry}/packages/package/${packagejson.name}/versions/${packagejson.version}`, {
    method: 'PUT',
    body: form,
    headers: {
      authorization: `Bearer ${opts.token}`,
