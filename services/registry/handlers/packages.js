...
async function packageList(context) {
  const [err, result] = await context.storageApi.listPackages({
...
async function packageDetail(
  ...
  const [err, str] = await context.storageApi.getPackage({namespace, host, name}).then(
    xs => [null, xs],
    xs => [xs, null]
  )
...
async function packageCreate(
  ...
  const [err, result] = await context.storageApi.replacePackage({
    host,
    namespace,
    name,
    require_tfa,
...
async function versionDetail(context, { host, namespace, name, version }) {
  const [err, result] = await context.storageApi.getPackageVersion({
    namespace,
    host,
    name,
    version
...
async function versionCreate(context, { host, namespace, name, version }) {
  const [err, result] = await context.storageApi.createPackageVersion({
    namespace,
    host,
    name,
    version,
...
async function versionDelete(context, { host, namespace, name, version }) {
  const [err, result] = await context.storageApi.deletePackageVersion({
    namespace,
    host,
    name,
    version,
...
async function getObject(context, { algo, '*': digest }) {
  const [err, stream] = await context.storageApi.getObject({algo, digest}).then(
    xs => [null, xs],
    xs => [xs, null]
  )
